...
struct diffusion_server {
    ...
    common_chat_params format_chat_request(const json & body) const {
        common_chat_templates_inputs inputs;
        inputs.messages = common_chat_msgs_parse_oaicompat(body.at("messages"));
        inputs.tools = common_chat_tools_parse_oaicompat(body.contains("tools") ? body.at("tools") : json());
        if (body.contains("tool_choice")) {
            const json & tool_choice = body.at("tool_choice");
            // The shared parser supports OpenAI's string choices. Treat an object choice
            // as a required tool call so clients that force a function still get a tool
            // constrained prompt instead of silently falling back to plain text.
            inputs.tool_choice = tool_choice.is_string()
                ? common_chat_tool_choice_parse_oaicompat(tool_choice.get<std::string>())
            ...
            for (int cur_step = n_steps; cur_step >= 1; --cur_step) {
                ...
                if (use_device_loop_step) {
                    ...
                    llama_diffusion_sample_params sample_params = {
                        ...
                        /* .cuda_final_tokens_on_stop = */ diffusion.cuda_final_tokens_on_stop && device_early_stop_interval > 0 && cur_step != 1,
...
static void scrub_tool_call_scaffold(common_chat_msg & msg) {
    if (msg.tool_calls.empty() || msg.content.empty()) return;
    if (msg.content.find("<|channel") == std::string::npos && msg.content.find("<channel|>") == std::string::npos) {
        return;
    }
    std::string s = msg.content;
    replace_all(s, "<|channel>thought", "");
    replace_all(s, "<|channel>", "");
    replace_all(s, "<channel|>", "");
    s = trim_copy(s);
    if (s == "thought") {
        s.clear();
...
static json parse_loose_tool_args(std::string raw) {
    raw = trim_copy(raw);
    ...
    for (const std::string & part : parts) {
        ...
        std::string key = strip_quotes(trim_copy(part.substr(0, colon)));
        std::string val = trim_copy(part.substr(colon + 1));
        if (key.empty()) continue;
        try {
            obj[key] = json::parse(val);
        } catch (...) {
            obj[key] = strip_quotes(val);
        }
    }
    return obj;
}

static bool parse_diffusion_gemma_text_tool_calls(const std::string & answer, common_chat_msg & msg) {
    static const std::regex call_re(R"(<\|tool_call>call:([A-Za-z0-9_.-]+)\s*(\{[\s\S]*?\})<tool_call\|>)");
    auto begin = std::sregex_iterator(answer.begin(), answer.end(), call_re);
    ...
    for (auto it = begin; it != end; ++it) {
        ...
        if ((size_t) m.position() > last) {
            msg.content += answer.substr(last, (size_t) m.position() - last);
        }
        common_chat_tool_call tc;
        tc.name = m[1].str();
        tc.arguments = parse_loose_tool_args(m[2].str()).dump();
        msg.tool_calls.push_back(std::move(tc));
        last = (size_t) m.position() + (size_t) m.length();
    }
    if (last < answer.size()) {
        msg.content += answer.substr(last);
    }
    msg.content = trim_copy(msg.content);
    return !msg.tool_calls.empty();
}

static common_chat_msg parse_chat_answer_oaicompat(const common_chat_params & chat_params, const std::string & answer) {
    common_chat_msg msg;
    try {
        ...
        msg = common_chat_parse(answer, false, parser_params);
    } catch (const std::exception & e) {
        SRV_WRN("failed to parse generated chat output, returning content only: %s\n", e.what());
    }

    if (msg.tool_calls.empty() && answer.find("<|tool_call>call:") != std::string::npos) {
        common_chat_msg fallback;
        if (parse_diffusion_gemma_text_tool_calls(answer, fallback)) {
            msg = std::move(fallback);
        }
    }
    ...
    msg.set_tool_call_ids(ids_cache, []() { return gen_id("call"); });
    scrub_tool_call_scaffold(msg);
    return msg;
}
...
int main(int argc, char ** argv) {
    ...
    auto models_list = [&model_obj]() {
        json models = json::array();
        models.push_back(model_obj());
        return json{{"object", "list"}, {"data", models}};
    };

    http.Get("/v1/models", [&models_list](const httplib::Request &, httplib::Response & res) {
        res.set_content(models_list().dump(), "application/json");
    });
    http.Get("/api/v1/models", [&models_list](const httplib::Request &, httplib::Response & res) {
        res.set_content(models_list().dump(), "application/json");
    });
    http.Get("/models", [&models_list](const httplib::Request &, httplib::Response & res) {
        res.set_content(models_list().dump(), "application/json");
    });
    http.Get(R"(/v1/models/.+)", [&model_obj](const httplib::Request &, httplib::Response & res) {
        res.set_content(model_obj().dump(), "application/json");
    });
    http.Get("/api/tags", [&srv](const httplib::Request &, httplib::Response & res) {
        json models = json::array({
            {
                {"name", srv.model_id},
                {"model", srv.model_id},
    ...
    http.Post("/api/show", [&srv](const httplib::Request &, httplib::Response & res) {
        res.set_content(json{
            ...
            {"model_info", {
                {"general.name", srv.model_id},
            }},
        }.dump(), "application/json");
    });

    // /props (llama-server analogue): server + model + default generation settings
    auto props_handler = [&](const httplib::Request &, httplib::Response & res) {
        json props{
    ...
    http.Get("/props", props_handler);
    http.Get("/v1/props", props_handler);
    ...
    http.Post("/v1/chat/completions", [&](const httplib::Request & req, httplib::Response & res) {
        ...
        try { chat_params = srv.format_chat_request(body); }
        catch (const std::exception & e) { res.status = 400; res.set_content(error_json(std::string("failed to format chat request: ") + e.what(), "invalid_request_error", 400).dump(), "application/json"); return; }
        // the chat template emits no BOS, so add it at tokenization (add_special)
        const std::vector<llama_token> prompt_tokens = common_tokenize(srv.vocab, chat_params.prompt, true, true);
        ...
        if (!stream) {
            json out{
                ...
                {"choices", json::array({ json{{"index", 0}, {"message", msg.to_json_oaicompat()}, {"finish_reason", finish_reason}} })},
                {"usage", usage_json(r)},
        ...
        res.set_chunked_content_provider("text/event-stream",
            [id, created, model_id, msg, finish_reason, timings, usage, include_usage](size_t, httplib::DataSink & sink) {
                ...
                if (!msg.content.empty() && !chunk(json{{"content", msg.content}}, nullptr, nullptr)) return false;
                for (size_t i = 0; i < msg.tool_calls.size(); ++i) {
                    const common_chat_tool_call & tc = msg.tool_calls[i];
                    json delta{
                    ...
                    if (!chunk(delta, nullptr, nullptr)) return false;
                }
                if (!chunk(json::object(), finish_reason.c_str(), &timings)) return false;
